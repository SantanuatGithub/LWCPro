...
var _a$2 = Node.prototype, appendChild = _a$2.appendChild, cloneNode = _a$2.cloneNode, compareDocumentPosition = _a$2.compareDocumentPosition, hasChildNodes = _a$2.hasChildNodes, insertBefore = _a$2.insertBefore, removeChild = _a$2.removeChild, replaceChild = _a$2.replaceChild;
...
var ShadowRootInnerHTMLSetter = typeof window.ShadowRoot !== 'undefined'
    ? getOwnPropertyDescriptor(window.ShadowRoot.prototype, 'innerHTML').set
    : function () {
        throw new Error('Internal Error: Missing ShadowRoot');
    };
...
function pathComposer(startNode, composed) {
    ...
    var startRoot = startNode === window ? window : patchedGetRootNode$1.call(startNode);
    while (current) {
...
function retarget(refNode, path) {
    ...
    for (var i = 0, ancestor = void 0, lastRoot = void 0, root = void 0, rootIdx = void 0; i < p$.length; i++) {
        ancestor = p$[i];
        root = ancestor === window ? window : patchedGetRootNode$1.call(ancestor);
        if (root !== lastRoot) {
...
var NodePatchDescriptors = {
    ...
    getRootNode: {
        ...
        value: function (options) {
            ...
            return isFalse(composed) ? this : patchedGetRootNode$1.call(getHost(this), { composed: composed });
        },
    },
};
...
function doesEventNeedsPatch(e) {
    ...
    if (originalTarget instanceof Node) {
        if ((compareDocumentPosition.call(document, originalTarget) &
            DOCUMENT_POSITION_CONTAINED_BY) !==
            0 &&
...
function focusOnNextOrBlur(focusEventTarget, segments) {
    muteFocusEventsDuringExecution(function () {
...
function keyboardFocusHandler(event) {
    ...
    if (position === 1) {
        ...
        var first_1 = getFirstTabbableMatch(segments.inner);
        if (!isNull(first_1)) {
            muteFocusEventsDuringExecution(function () {
                first_1.focus();
            });
...
var _a$a = OriginalMutationObserver.prototype, originalDisconnect = _a$a.disconnect, originalObserve = _a$a.observe, originalTakeRecords = _a$a.takeRecords;
// Internal fields to maintain relationships
var observedTargetsField = '$$lwcObservedTargets$$';
...
function filterMutationRecords(mutations, observer) {
    var observedTargets = observer[observedTargetsField];
    var observedTargetOwnerKeys = [];
    forEach.call(observedTargets, function (node) {
        // If the observed target is a shadowRoot, the ownerkey of the shadow tree will be fetched using the host
        var observedTargetOwnerKey = node instanceof window.ShadowRoot
            ? getNodeKey(node.host)
    ...
    return ArrayReduce.call(mutations, function (filteredSet, record) {
        ...
        if (type === 'childList' && !isUndefined(getNodeKey(target))) {
            ...
            var sampleNode = addedNodes.length > 0 ? addedNodes[0] : removedNodes[0];
            var sampleNodeOwnerKey = getNodeNearestOwnerKey(sampleNode);
            ...
            var recordTargetOwnerKey = target instanceof window.ShadowRoot
                ? getNodeKey(target.host)
                : getNodeNearestOwnerKey(target);
            var mutationInScope = ArrayIndexOf.call(observedTargetOwnerKeys, recordTargetOwnerKey) !== -1;
            if (mutationInScope) {
                ArrayPush.call(filteredSet, record);
            }
        }
...
function assertAttributeReflectionCapability(vm, attrName) {
    ...
    if (!isUndefined(getNodeOwnerKey$1(elm)) &&
...
function createTextHook(vnode) {
    var text = vnode.elm;
    var _a = vnode.owner, uid = _a.uid, fallback = _a.fallback;
...
function createCustomElmHook(vnode) {
    ...
    var uid = owner.uid, fallback = owner.fallback;
...
function createComponent(uninitializedVm, Ctor) {
    if (process.env.NODE_ENV !== 'production') {
        assert.isTrue(uninitializedVm && 'cmpRoot' in uninitializedVm, uninitializedVm + " is not a vm.");
    }
...
var _a$b = Array.prototype, ArraySlice$1 = _a$b.slice, ArrayUnshift$1 = _a$b.unshift, ArrayShift = _a$b.shift;
...
function createVM(tagName, elm, Ctor, options) {
    ...
    var def = getComponentDef(Ctor);
    var isRoot = options.isRoot, mode = options.mode, fallback = options.fallback, owner = options.owner;
...
function patchShadowRoot(vm, newCh) {
    ...
    if (newCh.length > 0 || oldCh.length > 0) {
        ...
        if (oldCh !== newCh) {
            var parentNode_1 = fallback ? elm : cmpRoot;
...
function getParentOrHostElement(elm) {
    ...
    return isNull(parentElement) && isNativeShadowRootAvailable$1
        ? getHostElement(elm)
        : parentElement;
}
/**
...
function createElement$2(sel, options) {
    ...
    var fallback = isUndefined(options.fallback) ||
    ...
    createVM(sel, element, Ctor, { mode: mode, fallback: fallback, isRoot: true, owner: null });
